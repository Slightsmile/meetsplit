            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-3">
                ...
                <Card className="bg-purple-50 border-purple-100">
                    <CardContent className="p-3 sm:p-4 text-center">
                        ...
                        <div className="text-xl sm:text-2xl font-bold text-slate-900">{bestDates.length}</div>
                        <div className="text-[10px] sm:text-xs text-slate-500">Dates</div>
                    </CardContent>
                </Card>
                <Card className="bg-green-50 border-green-100">
                    <CardContent className="p-3 sm:p-4 text-center">
                        <Receipt className="w-4 h-4 sm:w-5 sm:h-5 text-green-500 mx-auto mb-1" />
                        <div className="text-sm sm:text-2xl font-bold text-slate-900 break-words leading-tight">{formatMoney(totalExpenses)}</div>
                        <div className="text-[10px] sm:text-xs text-slate-500">Spent</div>
                    </CardContent>
                </Card>
            </div>
                    ...
                    {simplifiedDebts.length > 0 ? (
                        <div className="space-y-3">
                            {simplifiedDebts.map((debt, i) => (
                                <div key={i} className="flex items-center justify-between p-3 rounded-lg bg-slate-50 gap-2">
                                    <div className="flex items-center gap-2 min-w-0 flex-1">
                                        <div className="w-8 h-8 shrink-0 rounded-full bg-red-100 text-red-700 flex items-center justify-center font-bold text-xs">
                                            {getMemberName(debt.fromUser).substring(0, 2).toUpperCase()}
                                        </div>
                                        <span className="text-sm font-medium truncate">{getMemberName(debt.fromUser)}</span>
                                    </div>
                                    <div className="flex items-center gap-1.5 shrink-0">
                                        <span className="font-bold text-slate-900 text-sm whitespace-nowrap">{formatMoney(debt.amount)}</span>
                                        <ArrowRight className="w-4 h-4 text-slate-400 shrink-0" />
                                    </div>
                                    <div className="flex items-center gap-2 min-w-0 flex-1 justify-end">
                                        <span className="text-sm font-medium truncate">{getMemberName(debt.toUser)}</span>
                                        <div className="w-8 h-8 shrink-0 rounded-full bg-green-100 text-green-700 flex items-center justify-center font-bold text-xs">
